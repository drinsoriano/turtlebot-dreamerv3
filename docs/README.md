...
| [reward_shaping.md](reward_shaping.md) | The three reward modes — `default` (sparse), `shaped` (additive hand-weighted RS-1…RS-4), and `pbrs` (relative-distance + angular **potential-based reward shaping**) — with formulas and knobs, **why PBRS is policy-invariant / more theoretically grounded** than additive shaping, **why A\* is never used as a reward** (post-hoc metric only), the reward-component CSV schema, and a recommended default-vs-shaped-vs-pbrs experiment matrix. |
