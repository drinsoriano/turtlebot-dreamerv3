...
CSV_DIR    = CSV_BASE   # active CSV dir; sidebar may switch to a subfolder (e.g. tune_stage1)
...
def list_csv_folders() -> list[str]:
    ...
    BO trials write to csv_logs/tune_stage{N}/, so this surfaces those as
    selectable folders without mixing them into the main run list.
    """
    folders = ["."]
    if CSV_BASE.exists():
