...
- Gazebo RTF ≈ 4.5–4.7 (with GUI), ≈ 6.4 (headless). **Headless (`gui:=false`) is strongly recommended on the eGPU for stability** — `gzclient`'s OpenGL rendering competes with CUDA on the Thunderbolt link and can drop it mid-run (see Gazebo Launch Notes). Use headless for all long/BO runs.
- **`real_time_update_rate` raised 5000 → 7000 in the 8 stage world files (`worlds/turtlebot3_dqn_stage{1..8}/burger.model`), 2026-06-13.** Empirically gives a **stable ~6.4 headless RTF** (was ~4.8 at 5000); the 5000 throttle sat below the CPU's ~6.4 physics ceiling. `max_step_size` (0.001) is unchanged — only the throttle moved. **`-u 0` (unlimited) is intentionally avoided** — it was noisy (5.5–14) and dipped on every PNG-logging CPU burst because `gzserver` greedily grabbed all cores; the 7000 cap keeps headroom and stays stable. To change live without restarting: `gz physics -u 7000` (resets to the file value 7000 on the next launch). **Methodology rule: keep one RTF across all runs in a comparison — do not mix 5000hz and 7000hz runs** (RTF subtly shifts step↔sim-time, so the ablation is only apples-to-apples at a single rate; see [docs/step_distance_relationship.md](docs/step_distance_relationship.md)).
